...
impl<R: Runtime> DirMgr<R> {
    ...
    ///load directory from cache
    pub async  fn load_cache(self: &Arc<Self>) -> Result<()> {
        // Try to load from the cache.
        let attempt_id = AttemptId::next();
        info!("Trying to load a full directory from cache");
        let have_directory = self.load_directory(attempt_id).await?;
        info!("Load result : {}", have_directory);
