...
impl<R: Runtime> TorClient<R> {
    ...
    pub async fn load_cache(&self) -> crate::Result<()> {
        self.load_cache_inner().await.map_err(ErrorDetail::into)
    }

    async fn load_cache_inner(&self) -> StdResult<(), ErrorDetail> {
        // Load the cache from disk
        
        // Wait for an existing attempt to finish first.
        //
